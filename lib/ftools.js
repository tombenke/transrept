...
function getFileNames(path, extension)
{
    ...
    verbose && console.log('Read "' + extension + '" files from ' + path );
...
function processFiles( config ) {
    ...
    getFileNames(config.fromDir, config.fromExt).forEach( function( fileName ) {
            var fromFileName = path.resolve( config.fromDir, fileName + '.' + config.fromExt);
            var toFileName = path.resolve( config.toDir, fileName + '.' + config.toExt);
            verbose && console.log( '\nprocess\n  ' + fromFileName + '\n  >> ' + toFileName);
